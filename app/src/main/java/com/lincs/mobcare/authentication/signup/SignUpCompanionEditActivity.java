...
public class SignUpCompanionEditActivity extends AppCompatActivity {

    private EditText nameCompanion;
    private EditText phoneCompanion;
    private Spinner relationshipCompanion;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_edit_companion);
        ...
        nextButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(nameCompanion.getText().toString().equals("")){
                    Toast.makeText(getBaseContext(), "Digite o seu nome!", Toast.LENGTH_SHORT).show();
                }
                else if(phoneCompanion.getText().toString().equals("")){
                    Toast.makeText(getBaseContext(), "Digite o telefone!", Toast.LENGTH_SHORT).show();
                }
                else if(phoneCompanion.getText().length() < 14) {
                    Toast.makeText(getBaseContext(), "Telefone inválido!", Toast.LENGTH_SHORT).show();
                }
                else {
                    acompanhante.setNome(nameCompanion.getText().toString());
                    acompanhante.setTelefone(phoneCompanion.getText().toString());
                    acompanhante.setParentesco(relationshipCompanion.getSelectedItem().toString());
                    if (picturePath!=null) {
                        Firebase.updateAcompanhante(picturePath);
                    }
                    else{
                        Firebase.updateAcompanhante(acompanhante.foto);
                    }
                    finish();
        ...
        acompanhante = Snapshot.getAcompanhante();

        //Carrega imagem do acompanhante
        thumbnail=findViewById(R.id.thumbnail_edit_companion);
        LoadImg.loadImage(acompanhante.foto,thumbnail,this);

        nameCompanion = findViewById(R.id.etName);
        nameCompanion.setText(acompanhante.nome);

        phoneCompanion = findViewById(R.id.etTelefone);
        phoneCompanion.addTextChangedListener(new android.text.TextWatcher() {
            boolean editedFlag;
            ...
            public void afterTextChanged(Editable s) {
                ...
                if(!editedFlag) {
                    ...
                    if(!backspacingFlag && phoneLength > 11) {
                        phone = phone.substring(0,phoneLength-1);
                        phoneLength = phone.length();
                    }

                    String underline = "___________";
                    phone = phone + underline.substring(0,11-phoneLength);
                    newText = "(" + phone.substring(0,2) + ") " + phone.substring(2,7) + "-" + phone.substring(7,11);
                    phoneCompanion.setText(newText);

                    cursor++;
                    if(phoneLength > 2) cursor+=2;
                    if(phoneLength > 7) cursor++;
                    cursor+=phoneLength;
                    phoneCompanion.setSelection(cursor);
                }
                else {
                    editedFlag = false;
                }
        ...
        phoneCompanion.setText(acompanhante.telefone);

        relationshipCompanion = findViewById(R.id.spinner_family);
        SpinnerAdapter adapterView = relationshipCompanion.getAdapter();
        for (int i = 0; i < adapterView.getCount(); i++) {
            if (adapterView.getItem(i).equals(acompanhante.parentesco)) {
                relationshipCompanion.setSelection(i);
            }
        }
        ...
        thumbnail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int permissionCheck = ContextCompat.checkSelfPermission(SignUpCompanionEditActivity.this,
                        Manifest.permission.CAMERA);
                if (permissionCheck==PackageManager.PERMISSION_GRANTED) {
                    dispatchTakePictureIntent();
                }
                else {
                    ActivityCompat.requestPermissions(SignUpCompanionEditActivity.this,
    ...
    private void dispatchTakePictureIntent() {
        try {
            ...
            if (hasPerm == PackageManager.PERMISSION_GRANTED) {
                ...
                android.support.v7.app.AlertDialog.Builder builder = new android.support.v7.app.AlertDialog.Builder(SignUpCompanionEditActivity.this);
